...
const STATUS_COLORS: Record<string, string> = {
  ...
  completed: '#8B5CF6', cancelled_customer: '#EF4444', cancelled_store: '#EF4444', cancelled: '#EF4444'
};
...
export default function AdminDashboard() {
  ...
  if (!data) return <div className="flex items-center justify-center h-64"><div className="w-8 h-8 border-4 rounded-full animate-spin" style={{ borderColor: '#8B7355', borderTopColor: 'transparent' }} /></div>;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: '今日預約', value: data.todayTotal, icon: '📅', color: '#3B82F6' },
          { label: '待確認', value: data.pending, icon: '⏳', color: '#F59E0B' },
          { label: '已確認/到店', value: data.confirmed, icon: '👤', color: '#10B981' },
          { label: '已完成', value: data.completed, icon: '✓', color: '#8B5CF6' },
        ].map(s => (
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-white rounded-xl p-5 shadow-sm">
          <h2 className="font-semibold text-gray-800 mb-4">本週預約趨勢</h2>
          <div className="flex items-end gap-2 h-32">
            {data.weeklyTrend?.map((d: any, i: number) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full flex flex-col justify-end" style={{ height: '96px' }}>
                  <div className="w-full rounded-t-sm transition-all" style={{
                    height: `${(d.count / maxCount) * 90}%`,
          ...
          {data.staffWorkload?.map((s: any) => (
            <div key={s.username} className="mb-3">
              ...
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${Math.min((s.count / (data.todayTotal || 1)) * 100, 100)}%`, background: '#8B7355' }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <button onClick={fetchData} className="text-sm text-gray-400 hover:text-gray-600">↻ 刷新</button>
        </div>
