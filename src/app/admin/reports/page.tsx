  ...
  useEffect(() => {
    ...
    fetch(`/api/appointments?startDate=${start}&endDate=${end}`)
      .then(r => r.json()).then(apts => {
        ...
        apts.forEach((a: any) => { byService[a.service_name] = (byService[a.service_name] || 0) + 1; });
        setData({ total, completed, cancelled, revenue, byService });
      });
  }, [period]);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {[['week','近7天'],['month','近30天'],['year','近一年']].map(([v,l]) => (
            <button key={v} onClick={() => setPeriod(v)} className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${period === v ? 'text-white' : 'border border-gray-200 hover:bg-gray-50'}`}
              style={period === v ? { background: '#8B7355' } : {}}>
              {l}
            </button>
          ))}
        </div>
            ...
            {Object.entries(data.byService).sort(([,a],[,b]) => (b as number) - (a as number)).map(([name, count]) => (
              <div key={name} className="mb-3">
                <div className="flex justify-between text-sm mb-1">
                  <span>{name}</span><span className="text-gray-500">{count as number} 次</span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${((count as number) / data.total) * 100}%`, background: '#8B7355' }} />
                </div>
              </div>
            ))}
